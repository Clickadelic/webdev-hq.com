...
React auto-escapes JSX expressions by default. Never use `dangerouslySetInnerHTML` with untrusted content.
...
If you must render HTML, sanitize it server-side first.

## CSRF Protection

Inertia.js handles CSRF tokens automatically via the `X-XSRF-TOKEN` header. No manual `@csrf` tokens are needed in React components. Ensure the `VerifyCsrfToken` middleware is active (default in Laravel).
